...
class RandAugment(torch.nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        op_meta = RandAugment._augmentation_space(self.num_magnitude_bins, (height, width))
...
class TrivialAugmentWide(torch.nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        op_meta = TrivialAugmentWide._augmentation_space(self.num_magnitude_bins)
        op_index = int(torch.randint(len(op_meta), (1,)).item())
        op_name = list(op_meta.keys())[op_index]
        magnitudes, signed = op_meta[op_name]
        magnitude = (
...
def augmentation_space(num_bins: int=31, image_size: Tuple[int, int]=[320,320]) -> Dict[str, Tuple[Tensor, Tensor, bool]]:
    return {
        "Identity": (torch.tensor(0.0), torch.linspace(0.0, 1, num_bins), False),
        "ShearX": (torch.linspace(0.0, 1, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "ShearY": (torch.linspace(0.0, 1, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "TranslateX": (torch.linspace(0.0, image_size[1], num_bins), torch.linspace(0.0, 1, num_bins), True),
        "TranslateY": (torch.linspace(0.0, image_size[0], num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Rotate": (torch.linspace(0.0, 180.0, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Brightness": (torch.linspace(0.0, 0.9, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Color": (torch.linspace(0.0, 0.9, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Contrast": (torch.linspace(0.0, 0.9, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Sharpness": (torch.linspace(0.0, 0.9, num_bins), torch.linspace(0.0, 1, num_bins), True),
        "Posterize": (8 - (torch.arange(num_bins) / ((num_bins - 1) / 4)).round().int(), False),
        "Solarize": (torch.linspace(255.0, 0.0, num_bins), False),
        "AutoContrast": (torch.tensor(0.0), False),
        "Equalize": (torch.tensor(0.0), False),
...
class MyAugment(torch.nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        aug_space = augmentation_space(self.num_magnitude_bins, (height, width))
        for p in self.policy:
            op_index, magnitude_index, prob_index = list(*p)
