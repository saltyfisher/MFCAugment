...
def augmentation_space(mag_bins: int=31, prob_bins: int=10, image_size: Tuple[int, int]=[320,320]) -> Dict[str, Tuple[Tensor, Tensor, bool]]:
    return {
        ...
        # 颜色空间操作
        "Brightness": (torch.linspace(0.0, 2.0, mag_bins), torch.linspace(0.0, 1, prob_bins), True),
        "Color": (torch.linspace(0.0, 2.0, mag_bins), torch.linspace(0.0, 1, prob_bins), True),
        "Contrast": (torch.linspace(0.0, 2.0, mag_bins), torch.linspace(0.0, 1, prob_bins), True),
        "Sharpness": (torch.linspace(0.0, 2.0, mag_bins), torch.linspace(0.0, 1, prob_bins), True),
