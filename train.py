...
def train_val(gpu_id, task_id, args, config, itrs, dataroot, save_path=None, only_eval=False, only_train=False):
    ...
    for epoch in tqdm(range(epoch_start, max_epoch + 1),desc=f"Task{task_id} Iter{itrs}",position=task_id,dynamic_ncols=True,leave=True):
        model.train()
        ...
        if only_eval or epoch % 20 == 0 or epoch == max_epoch:
            ...
            if rs['test'][-1]['f1'] > best_f1:
                ...
                optimal_params['model'] = deepcopy(model)
                optimal_params['optimizer'] = deepcopy(optimizer)
                optimal_params['epoch'] = epoch
            ...
            if save_path and config.get('save_model', True):
                ...
                torch.save({
                    ...
                    'optimizer': optimal_params['optimizer'].state_dict(),
                    'model': optimal_params['model'].state_dict(),
                    'update_epoch':optimal_params['epoch']
                }, save_path+'.pth')    
...
if __name__ == '__main__':
    ...
    if 'lym' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('lym*'))
    elif 'lc' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('lc*'))
    elif 'breakhis40X' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('*40X*'))
    elif 'breakhis100X' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('*100X*'))
    elif 'breakhis200X' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('*200X*'))
    elif 'breakhis400X' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('*400X*'))
    elif 'rect' in args.dataset:
        all_config_files = list(Path('/workspace/Code/ImageAugment/MFCAugment/networks/confs').glob('rect*'))
    ...
    for itrs in range(10):        
        for args, cfg in zip(all_args, all_config):
            ...
            #     continue
            print(save_name)
            train_val(0, args.device, args, cfg, itrs, '../data','./params_save')
